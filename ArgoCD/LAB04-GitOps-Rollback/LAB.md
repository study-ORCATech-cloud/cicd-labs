...
**1. Prepare Your Git Repository:**
   * Use your existing Git repository from previous labs, or create a new public repository named `rollback-demo`
...
**2. Deploy Version 1 (Stable Release):**
   * Copy the v1 deployment file from the lab materials:
      ```bash
      ...
      ```
   * Review the v1 deployment file - it's a stable nginx deployment with 2 replicas
...
**5. Synchronize Version 1:**
   * Click on your `rollback-demo` application in ArgoCD
   * Click **"SYNC"** and then **"SYNCHRONIZE"**
   * Wait for the application to become `Healthy` and `Synced`
...
**7. Update to Version 2 (Problematic Version):**
   ...
   * Commit and push this problematic version:
      ```bash
      git add deployment.yaml
...
**8. Synchronize Version 2:**
   * Go back to ArgoCD UI - your application should show `OutOfSync`
   * Click **"SYNC"** and **"SYNCHRONIZE"**
   * Watch the deployment process
   * Notice that some pods might remain in `Pending` state due to resource constraints
...
**9. Observe the Problem:**
   * Check the pod status:
      ...
      kubectl get pods -n rollback-demo-app
      ```
   * You should see some pods in `Pending` state due to insufficient cluster resources
   * Describe a pending pod to see the scheduling error:
      ```bash
      kubectl describe pod <POD_NAME> -n rollback-demo-app
      ```
   * Look for events showing `Insufficient memory` or `Insufficient cpu`
   * Your application is having performance issues and needs to be rolled back!

### Phase 3: ArgoCD UI Rollback

**10. View Application History:**
   * In the ArgoCD UI, click on your `rollback-demo` application
   * Click on the **"HISTORY AND ROLLBACK"** tab (or **"History"** button)
   ...
   * Note the Git commit IDs and sync timestamps

**11. Perform UI-Based Rollback:**
   * In the History view, find Revision 1 (the working version)
   * Click the **"ROLLBACK"** button next to Revision 1
   * Confirm the rollback operation
   * ArgoCD will automatically sync back to the previous working state
   * Wait for the application to become `Healthy` and `Synced` again
...
**12. Verify the Rollback:**
   * Check that the pods are running correctly:
      ...
      kubectl get pods -n rollback-demo-app
      ```
   * You should see 2 pods (back to v1 configuration) and they should be `Running`
   * Access the application again via browser to confirm it's working
   * Check the ArgoCD UI - it should show `Synced` and `Healthy`

### Phase 4: Git-Based Rollback Approach

**13. Deploy Version 3 (Fixed Version):**
   * Copy the v3 deployment file:
      ```bash
      cp ../path-to-cicd-labs/ArgoCD/LAB04-GitOps-Rollback/app-versions/v3-deployment.yaml ./deployment.yaml
      ```
   * Review v3 improvements:
      - Uses a stable nginx image (`nginx:1.22`)
      - 3 replicas (balanced)
      - Added health checks
   * Commit and push version 3:
      ```bash
      git add deployment.yaml
...
**14. Sync Version 3:**
   * In ArgoCD UI, sync the new version
   * Verify the deployment is healthy with 3 replicas
   * Check that the health checks are working:
      ```bash
      kubectl describe deployment rollback-demo-app -n rollback-demo-app
      ```

**15. Simulate Another Problem (Git Rollback Method):**
   * Let's say v3.0.0 has a critical issue that wasn't caught immediately
   * Instead of using ArgoCD rollback, we'll use Git to revert
...
**16. Observe Git-Based Rollback:**
   * In ArgoCD UI, your application should show `OutOfSync` 
   * Click **"SYNC"** and **"SYNCHRONIZE"**
   * ArgoCD will now deploy the reverted state
   * However, you'll notice we're back to the broken v2 (this is intentional to show the difference)

**17. Fix with Proper Git Rollback:**
   * Since v2 was also broken, let's revert to v1 properly:
      ```bash
      git revert HEAD    # Revert the revert (which gets us back to v3)
      git revert HEAD~2  # Revert the v2 commit
      ```
   * Or alternatively, use Git reset to go back to v1:
      ```bash
      git log --oneline  # Find the v1 commit hash
      git reset --hard <v1-commit-hash>
      git push --force-with-lease origin main
      ```
   * Sync in ArgoCD to get back to the stable v1 state

### Phase 5: ArgoCD CLI Rollback (Optional)

**18. Install ArgoCD CLI (If Not Already Installed):**
   * Download ArgoCD CLI for your OS from: https://argo-cd.readthedocs.io/en/stable/cli_installation/
   * For Linux/macOS:
      ```bash
...
**19. Login to ArgoCD CLI:**
   * Get your ArgoCD admin password:
      ```bash
...
**3. Clean Up Git Repository (Optional):**
   ...
   * Or clean up the commits if desired:
      ```bash
