...
**1. Prepare Your Git Repository:**
   * Use your existing Git repository from previous labs, or create a new public repository named `sync-policy-demo`
...
**2. Copy the Demo Application Files:**
   * Copy all the sync demo files from the lab materials:
      ```bash
      ...
      ```
   * Review the files - they create a simple nginx deployment with configurable settings
...
**5. Test Manual Sync Behavior:**
   * Your application should appear as `Missing` and `OutOfSync`
   * Notice that nothing happens automatically - this is manual sync behavior
   * Click **"SYNC"** and **"SYNCHRONIZE"** to deploy
...
**6. Test Manual Sync with Changes:**
   * Make a change to the ConfigMap in your local repository:
      ```bash
   ...
   * Commit and push the change:
      ...
      git push origin main
      ```
   * Go back to ArgoCD UI - after a few minutes, the application should show `OutOfSync`
   * **Important:** Notice that the change is NOT automatically applied
   * You must manually click **"SYNC"** to apply the change

### Phase 2: Enable Auto Sync

**7. Configure Auto Sync via ArgoCD UI:**
   * In the ArgoCD UI, click on your `sync-policy-demo` application
   * Click on **"APP DETAILS"** (top-left area or details button)
   * In the summary section, click **"EDIT"** 
   * Scroll down to find **"SYNC POLICY"**
   * Toggle **"AUTOMATED"** to enable auto-sync
   * Click **"SAVE"**
   * You should see the sync policy now shows as **"Automated"**

**8. Test Auto Sync Behavior:**
   * Make another change to test auto-sync:
      ```bash
   ...
   * Commit and push:
      ...
      git push origin main
      ```
   * Watch the ArgoCD UI - within 1-3 minutes, you should see:
...
**9. Enable Self-Healing:**
   * Go back to your application in ArgoCD UI
   * Click **"APP DETAILS"** → **"EDIT"**
   * In the **"SYNC POLICY"** section, enable **"SELF HEAL"**
   * Click **"SAVE"**

**10. Test Configuration Drift (Manual Changes):**
   * Manually scale the deployment using kubectl:
      ```bash
   ...
   * Check the pods immediately:
      ...
      kubectl get pods -n sync-demo-app
      ```
   * You should see 5 pods for a short time
   * Watch the ArgoCD UI - within 1-3 minutes, you should see:
...
**11. Test Manual Resource Deletion:**
   ...
   * Wait 1-3 minutes and check again:
      ...
      kubectl get configmap -n sync-demo-app
      ```
   * The ConfigMap should be automatically recreated by self-healing!

### Phase 4: Automated Pruning

**12. Enable Automated Pruning:**
   * In ArgoCD UI, edit your application again
   * In the **"SYNC POLICY"** section, enable **"PRUNE RESOURCES"**
   * Click **"SAVE"**

**13. Test Automated Pruning:**
   * Add a temporary resource to your Git repository:
      ```bash
   ...
   * Commit and push:
      ...
      git push origin main
      ```
   * Wait for auto-sync to deploy the new resource:
      ```bash
      kubectl get configmap -n sync-demo-app
      ```
   * You should see both `sync-demo-config` and `temp-config`
   * Now remove the temporary resource from Git:
      ```bash
      git rm temp-config.yaml
      git commit -m "Remove temporary resource - test pruning"
      git push origin main
      ```
   * Wait for auto-sync and check:
      ```bash
      kubectl get configmap -n sync-demo-app
      ```
   * The `temp-config` ConfigMap should be automatically removed (pruned)!

### Phase 5: Sync Options and Advanced Configuration

**14. Configure Sync Options:**
   * Edit your application in ArgoCD UI one more time
   * Look for **"SYNC OPTIONS"** (may be in an advanced section)
...
**15. Test Complete Automation Workflow:**
   ...
   * Commit and push all changes:
      ...
      git push origin main
      ```
   * Watch the ArgoCD UI for automatic synchronization
...
**16. Compare Manual vs Auto Sync:**
   * Temporarily disable auto-sync to compare:
...
**17. Access Your Application:**
   * Get your Minikube IP:
      ...
      minikube ip
      ```
   * Open browser to: `http://<MINIKUBE_IP>:30095`
